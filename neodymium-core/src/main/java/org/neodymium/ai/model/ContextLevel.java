...
public enum ContextLevel
{
    ...
    /**
     * Cleans and sanitizes a raw context level string.
     * Safely normalizes unparseable strings, returning the fallback or LEAN if null.
     *
     * @param rawName the raw level name from model response or configuration
     * @param fallback the default fallback level if parsing fails or input is null
     * @return the cleaned, validated ContextLevel
     */
    public static ContextLevel clean(final String rawName, final ContextLevel fallback)
    {
        final ContextLevel baseFallback = fallback != null ? fallback : LEAN;
        return fromString(rawName, baseFallback);
    }

    /**
     * Cleans and validates a ContextLevel, defaulting to LEAN if null.
     *
     * @param level the candidate context level
     * @return the cleaned, validated ContextLevel
     */
    public static ContextLevel clean(final ContextLevel level)
    {
        return level != null ? level : LEAN;
    }
}
