...
public enum SemanticIntent
{
    ...
    /**
     * Infers a probable {@link SemanticIntent} from an instruction text.
     *
     * @param instruction the natural language instruction text
     * @return the inferred intent, or null if ambiguous
     */
    public static SemanticIntent inferFromInstruction(final String instruction)
    {
        if (instruction == null || instruction.isBlank())
        {
            return null;
        }
        final String lower = instruction.trim().toLowerCase();
        if (lower.startsWith("open ") || lower.startsWith("navigate ") || lower.startsWith("go to http") || lower.startsWith("browse "))
        {
            return NAVIGATE;
        }
        if (lower.startsWith("click ") || lower.contains(" click ") || lower.contains(" and click") || lower.startsWith("press ") || lower.contains(" press "))
        {
            return CLICK;
        }
        ...
        if (lower.startsWith("hover ") || lower.startsWith("scroll ") || lower.contains(" scroll "))
        {
            return HOVER_SCROLL;
        }
        if (lower.startsWith("wait ") || lower.startsWith("pause ") || lower.startsWith("sleep "))
        {
            return WAIT;
        }
        if (lower.startsWith("validate ") || lower.startsWith("verify ") || lower.startsWith("assert ") || lower.startsWith("check ")
                || lower.contains(" is now ") || lower.contains(" says now ") || lower.contains(" are now ")
                || lower.contains(" is shown") || lower.contains(" is displayed") || lower.contains(" is visible")
                || lower.contains(" is mentioned") || lower.contains(" should be ") || lower.contains(" must be "))
        {
            return ASSERT;
        }
        return null;
