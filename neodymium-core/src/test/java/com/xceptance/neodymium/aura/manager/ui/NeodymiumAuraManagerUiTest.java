...
public class NeodymiumAuraManagerUiTest
{
    ...
    private static void cleanStaleDummyTestFiles()
    {
        ...
        final File sandboxDir = new File("src/test/java/com/xceptance/neodymium/aura/sandbox").getAbsoluteFile();
        if (sandboxDir.exists())
        {
            final File[] sandboxFiles = sandboxDir.listFiles((dir, name) -> name.startsWith("Aura_dummy_test_run_") && name.endsWith(".java"));
            if (sandboxFiles != null)
            {
                for (final File file : sandboxFiles)
                {
                    try
                    {
                        Files.deleteIfExists(file.toPath());
