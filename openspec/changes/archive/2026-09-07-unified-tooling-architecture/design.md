...
### 11. Visual Identification Fallback and The Re-Anchoring Bridge
- **Decision**: Provide robust visual fallback for ambiguous, textless, or non-DOM elements (such as icon-only SVG buttons, canvas components, or obfuscated styling) through a coordinated two-tier strategy coupled with an automatic DOM re-anchoring bridge:
  1. **Tier 2 Offline Perceptual Visual Matching (0 LLMs, <1ms)**: `DomFeatureVector` stores spatial bounding boxes $(x, y, w, h)$, aspect ratio, and an element-level perceptual dHash / tile SSIM matrix computed via `ScreenshotHasher.computeTileSsimMatrix(...)`. During offline replay, if candidate text is absent or selectors fail, Tier 2 compares candidate element crops against the recorded perceptual hash locally on CPU. If visual similarity $\ge 0.85$, it heals the element in $< 1\text{ms}$ and feeds it to Tier 1 Selenide without invoking any LLM.
  2. **Tier 3 Multimodal Visual Discovery Tools**:
     - `browser_inspect_visual(selector, prompt)`: Captures a targeted bounding-box crop of the requested selector/container, returning a compact base64 thumbnail (~80–150 tokens) to visually inspect icons, layout, or graphical cues with minimal token overhead.
     - `browser_take_screenshot(mark_interactive=true)`: Injects Set-of-Marks (SoM) temporary visual numeric badges (`[1]`, `[2]`, `[3]`) over interactive candidate centers, allowing the Vision LLM to resolve visual ambiguity or interact with non-DOM elements.
  3. **The Re-Anchoring Bridge (`document.elementFromPoint`)**: Whenever a visual badge or coordinate interaction occurs, Neodymium executes `PageAnalyzer.resolveAnchorCoordinate(driver, x, y)` via `document.elementFromPoint(x, y)` to discover the real underlying DOM element at those coordinates. It extracts a stable hierarchical selector and `DomFeatureVector`, storing them in the recorded `PlaybookStep`. Visual fallback runs at most once during authoring; subsequent executions replay deterministically at native machine speed in Tier 1 or Tier 2.
- **Rationale**: Eliminates the token bloat of sending full-viewport screenshots by default, while guaranteeing that visual interactions are seamlessly converted back into deterministic, offline-replayable DOM locators.
- **Alternatives Considered**: Storing raw pixel coordinates permanently in playbooks. Rejected because viewport scaling, responsive breakpoints, or minor layout shifts break raw coordinate playback.
