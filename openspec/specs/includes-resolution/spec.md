# Includes Resolution Specification

## Purpose
Define the syntax, parsing pipeline, scope resolution rules, and error handling for static and dynamic inclusions (`_include:`) within YAML test scripts and datasets in the Neodymium library.

---

## Static vs. Dynamic Inclusions

To ensure absolute clarity in how inclusions are processed and executed, the framework strictly distinguishes between **Static Inclusions** and **Dynamic Inclusions**:

| Feature / Aspect | Static Inclusions | Dynamic Inclusions |
| :--- | :--- | :--- |
| **Declaration Location** | Root lifecycle blocks (`_beforeAll`, `_afterAll`, `_beforeEach`, `_steps`, etc.) or root `data` / dataset rows. | Within conditional step statements (e.g., `If ... then _include: ... else ...`) or dynamically evaluated loops. |
| **Resolution Time** | Parsed and expanded at compile/parse time (before test execution begins). | Expanded inline during the variable/inclusion resolution phase, but evaluated at runtime. |
| **Execution Context** | Spliced directly in-place into the test script step hierarchy. | Sent as part of the step text to the LLM, which dynamically decides which branch to execute. |
| **Logic Evaluation** | Deterministic and unconditional. | Conditional and state-dependent, interpreted dynamically by the LLM based on page state. |
| **Java Pattern Parsing** | None (spliced directly by the YAML parser). | None (no regular expression or condition matching is done in Java; the LLM handles all logic). |
