...
public class RecordingDurationWithAlertTest extends AbstractNeodymiumTest {
	...
	public double runTest(boolean isGif, String oneImagePerMilliseconds) throws IOException, InterruptedException {
		CustomRecordingWithAlertTest.isGif = isGif;
		...
		recordingFile = new File(
				config.tempFolderToStoreRecording() + CustomRecordingWithAlertTest.uuid + "." + config.format());
		...
		ProcessBuilder pb = new ProcessBuilder("ffprobe", "-v", "error", "-show_entries", "format=duration", "-of",
				"default=noprint_wrappers=1:nokey=1", recordingFile.getAbsolutePath());
