...
/**
 * Live LLM integration test for the Shadow DOM Piercing sandbox challenge.
 * Executes live against Gemini model across 3 modes.
 *
...
@Tag("LiveLlm")
@NeodymiumAiTest
@AiPlaybook(value = "programmatic", recordingFileName = "live_shadow_dom_playbook")
public class ShadowDomSandboxLiveTest extends BaseAiTest
{
    ...
    public void setupProperties(final AiSession session)
    {
        ...
        session.getExecutionContext().getSessionData().putDynamic("shadow.test.url", pageUrl, false);
    }

    /**
     * Tests Shadow DOM challenge live across 3 execution modes.
     *
     * @param session the thread-isolated AiSession
     */
    @AiPlaybook
    @AiMode({ExecutionMode.FORCE_RECORDING, ExecutionMode.REPLAY_STRICT, ExecutionMode.REPLAY_WITH_HEALING})
    public void testShadowDomLive(final AiSession session)
    {
        runPlaybook(session, """
            steps: |
