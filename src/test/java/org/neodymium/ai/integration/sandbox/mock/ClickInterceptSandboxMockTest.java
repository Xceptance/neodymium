...
/**
 * Mock integration test for the Click Interception sandbox challenge.
...
@Tag("AuraIntegration")
@NeodymiumAiTest
@AiPlaybook(value = "programmatic", recordingFileName = "custom_click_intercept_playbook")
public class ClickInterceptSandboxMockTest extends BaseAiTest
{
              ...
              "actions": [
                {
                  ...
                  "locator": "",
                  "value": "%s",
                  "reasoning": "Navigate to Click Intercept challenge page"
                }
              ]
            }
            """.formatted(pageUrl), null, "mock"));

        // 2. Click blocking overlay to dismiss it
        mock.addResponse(new LlmResponse("""
            {
              "actions": [
                {
                  "action": "CLICK",
                  "locator": "#blocking-overlay",
                  "value": "",
                  "reasoning": "Dismiss blocking overlay"
                }
              ]
            }
            """, null, "mock"));

        // 3. Click transaction button
        mock.addResponse(new LlmResponse("""
            {
              "actions": [
                {
                  "action": "CLICK",
                  "locator": "#intercepted-btn",
                  "value": "",
                  "reasoning": "Click transaction submit button"
                }
              ]
            }
            """, null, "mock"));
        ...
        mock.addResponse(new LlmResponse("""
            {
              "actions": [
                {
                  "action": "ASSERT",
                  "locator": "#intercept-status",
                  "value": "Transaction Successful!",
                  "reasoning": "Verify transaction success status message"
                }
              ]
            }
            """, null, "mock"));
    ...
    /**
     * Tests Click Intercept challenge across 3 execution modes.
     *
     * @param session the thread-isolated AiSession
     */
    @AiPlaybook
    ...
    public void testClickInterceptMock(final AiSession session) throws Exception
    {
        session.execute( """
            steps: |
              ...
              Click #intercepted-btn
              Verify that #intercept-status shows "Transaction Successful!"
            """);

        $("#intercept-status").shouldHave(text("Transaction Successful!"));
    }
}
