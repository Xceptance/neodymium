...
/**
 * Live LLM integration test for the Multi-level Hover Chain sandbox challenge.
...
@Tag("LiveLlm")
@NeodymiumAiTest
@AiPlaybook(value = "programmatic", recordingFileName = "live_hover_chain_playbook")
public class HoverChainSandboxLiveTest extends BaseAiTest
{
    ...
    public void setupProperties(final AiSession session)
    {
        ...
        session.getExecutionContext().getSessionData().putDynamic("hover.chain.test.url", pageUrl, false);
    }

    /**
     * Tests multi-level hover chain challenge using live LLM across 3 execution modes.
     *
     * @param session the thread-isolated AiSession
     */
    @AiPlaybook
    @AiMode({ExecutionMode.FORCE_RECORDING, ExecutionMode.REPLAY_STRICT, ExecutionMode.REPLAY_WITH_HEALING})
    public void testHoverChainLive(final AiSession session)
    {
        runPlaybook(session, """
            steps: |
