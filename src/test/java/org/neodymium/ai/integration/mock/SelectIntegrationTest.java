...
/**
 * Mock programmatic integration test for the SELECT action plugin.
 *
...
@Tag("AuraIntegration")
@NeodymiumAiTest
@AiPlaybook(value = "programmatic", name = "custom_select_playbook")
public class SelectIntegrationTest extends BaseAiTest
{
    ...
    public void setupPropertiesAndMock(final AiSession session)
    {
        ...
        session.getExecutionContext().getSessionData().putDynamic("select.test.url", pageUrl, false);
              ...
              "actions": [
                {
                  ...
                  "locator": "",
                  "value": "%s",
                  "reasoning": "Navigate to select test page"
                }
              ]
            }
    ...
    public void testSelectMock(final AiSession session)
    {
        runPlaybook(session, """
            data:
              - testId: selectData
        ...
        try
        {
            ...
            org.junit.jupiter.api.Assertions.assertTrue(content.contains("\"target\" : \"${select.test.url}\""), 
