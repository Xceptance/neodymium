...
/**
 * Mock programmatic integration test for the CHECK action plugin.
 *
...
@Tag("AuraIntegration")
@NeodymiumAiTest
@AiPlaybook(value = "programmatic", name = "custom_check_playbook")
public class CheckIntegrationTest extends BaseAiTest
{
    ...
    public void setupPropertiesAndMock(final AiSession session)
    {
        ...
        session.getExecutionContext().getSessionData().putDynamic("check.test.url", pageUrl, false);
    ...
    public void testCheckMock(final AiSession session)
    {
        runPlaybook(session, """
            data:
              - testId: checkData
        ...
        try
        {
            ...
            org.junit.jupiter.api.Assertions.assertTrue(content.contains("\"target\" : \"${check.test.url}\""), 
