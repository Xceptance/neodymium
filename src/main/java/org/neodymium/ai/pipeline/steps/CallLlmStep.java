...
public final class CallLlmStep<T> implements PipelineStep
{
    ...
    private void executeInternal(final ExecutionContext context) throws PipelineException
    {
        ...
        final ResponseSchema schema = this.prompt.getResponseSchema();
        final ReasoningEffort reasoningEffort = ResponseSchema.resolveReasoningEffort(schema);
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            try
            {
                ...
                LOGGER.debug("LLM response received (attempt {}/{}). Length: {} chars (duration: {} ms)",
                    attempt, maxAttempts, response.content() != null ? response.content().length() : 0, durationMs);
                if (LOGGER.isTraceEnabled())
                {
            ...
            catch (final Exception e)
            {
                ...
                {
                    LOGGER.warn("⚠️ Failed to parse LLM response on attempt {}/{}: {}. Retrying query once...", attempt, maxAttempts, e.getMessage());
                    continue;
                }
                LOGGER.error("Failed to parse LLM response into target signature after {} attempts. Raw response: {}", maxAttempts, response.content(), e);
                throw new ConclusiveFailureException("Failed to parse LLM response into target signature after retry", e);
            }
        }
    }
