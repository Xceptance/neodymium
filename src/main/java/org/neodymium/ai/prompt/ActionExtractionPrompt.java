...
public final class ActionExtractionPrompt implements AiPrompt<List<Action>>
{
    ...
    public String compileSystemMessage(final ExecutionContext context)
    {
        final String basePrompt = """
               ...
               - Set 'locator' to the CSS selector provided in the element's 'selector="..."' or 'id="..."' attribute in the DOM state. CRITICAL FOR OFFLINE REPLAY: The 'locator' value is saved directly into YAML playbooks for offline execution by Selenide/WebDriver. Therefore, 'locator' MUST be a valid, real CSS selector matching the element's 'selector="..."' or 'id="..."' attribute (or a valid text selector). NEVER output synthetic pseudotags (such as 'text:nth-of-type(N)') or broad outer containers (such as 'div.container', 'body', 'html'). If an element in the text dump lacks a specific 'selector="..."' or 'id="..."' attribute, set 'status' to 'ESCALATE' to request visual context.
