...
public final class ActionExtractionPrompt implements AiPrompt<List<Action>>
{
    ...
    public String compileSystemMessage(final ExecutionContext context)
    {
        final String basePrompt = """
               ...
               - Set 'locator' to a specific, robust CSS selector targeting the precise DOM element, or 'url' to verify current page URL. CRITICAL: DOM dump tags represent real HTML elements (e.g. <p>, <div>, <span>, <h1>, <button>, <input>, <link>). Never invent synthetic tag names or pseudotags (such as 'text' or 'text:nth-of-type(N)') in locators, and never select broad outer layout containers (such as 'div.container', '.wrapper', 'body', 'html'). Always resolve a specific CSS selector targeting the actual element or its immediate parent container (e.g., 'p.order-total', 'div:has(...)'). If a specific CSS selector is not available in the current context, set 'status' to 'ESCALATE' to request visual context.
