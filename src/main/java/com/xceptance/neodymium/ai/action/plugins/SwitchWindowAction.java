...
public class SwitchWindowAction implements AiActionPlugin
{
    ...
    public final void execute(final Action action, final Object testInstance, final ActionExecutor executor)
    {
        ...
        {
            final String currentHandle = Neodymium.interaction().getWindowHandle();
            final Set<String> handles = Neodymium.interaction().getWindowHandles();
            ...
            if (targetHandle != null)
            {
                ...
                Neodymium.interaction().switchToWindow(targetHandle);
            }
            else
            {
        ...
        else
        {
            ...
            try
            {
                ...
                if (index >= 0 && index < handleList.size())
                {
                    ...
                    Neodymium.interaction().switchToWindow(handleList.get(index));
                }
                else
                {
            ...
            catch (final NumberFormatException e)
            {
                ...
                if (handles.contains(windowParam))
                {
                    ...
                    Neodymium.interaction().switchToWindow(windowParam);
                    return;
                }
                ...
                {
                    Neodymium.interaction().switchToWindow(handle);
                    final String pageTitle = Neodymium.interaction().title();
                    if (pageTitle.equalsIgnoreCase(windowParam) || pageTitle.contains(windowParam))
                    {
                ...
                Neodymium.interaction().switchToWindow(currentHandle);
