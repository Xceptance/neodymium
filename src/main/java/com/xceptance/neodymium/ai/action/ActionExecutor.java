...
public class ActionExecutor {
    ...
    /**
     * Maps stale/recorded window handles to active window handles for robust
    ...
    private Action cloneActionForExecution(final Action source) {
        final Action clone = new Action();
        clone.setType(source.getType());
        clone.setTarget(source.getTarget());
        if (source.getValues() != null) {
            clone.setValue(new java.util.ArrayList<>(source.getValues()));
        }
        clone.setDescription(source.getDescription());
        clone.setElementDetails(source.getElementDetails());
        clone.setReasoning(source.getReasoning());
        clone.setFrameId(source.getFrameId());
        clone.setAdjust(source.isAdjust());
        clone.setSilent(source.isSilent());
        if (source.getElementContext() != null) {
            clone.setElementContext(new java.util.HashMap<>(source.getElementContext()));
        }
        return clone;
    }
    ...
    public void execute(final Action action) {
        ...
        final Action interpolatedAction = cloneActionForExecution(action);
        interpolateAction(interpolatedAction);
        switchFrameContext(interpolatedAction.getFrameId());

        preCheckAction(interpolatedAction);

        AiActionPlugin plugin = ActionRegistry.getPlugin(interpolatedAction.getType());

        try {
            if (plugin != null) {
                try {
                    plugin.execute(interpolatedAction, test, this);

                    if (interpolatedAction.getElementContext() != null && !interpolatedAction.getElementContext().isEmpty()) {
                        LOG.debug("   ✅ Interacted with element: {}", interpolatedAction.getElementContext());
                        action.setElementContext(interpolatedAction.getElementContext()); // Keep context for logging
                    }
                } finally {
                    plugin.cleanup(interpolatedAction, this);
                }
            } else {
                LOG.warn("Unsupported action type: {}", interpolatedAction.getType());
            }
        } finally {
            try {
    ...
    public SelenideElement findElement(final Action action) {
        try {
            ...
            throw new ActionExecutionException(
                    String.format("Could not find element for target '%s' or text '%s'", action.getTarget(),
                            action.getElementDetails()));
        }
    }

    /**
     * Finds all elements using the same strategies as findElement, polling until
    ...
    private ElementsCollection findElementsInternal(final Action action, final boolean logErrors) {
            ...
            try {
                ...
                if (!elements.isEmpty()) {
                    ...
                    if (shadowWebEls != null && !shadowWebEls.isEmpty()) {
                        logDebug(logErrors,
                                "   🔍 Resolved using Strategy 0 (Shadow DOM JS): Neodymium Automation ID [{}]",
                                target);
                        return Selenide.$$(shadowWebEls);
                    }
                    logDebug(logErrors, "   ❌ Strategy 0 failed: Neodymium Automation ID [[data-neo-ref='{}']]",
                            target);
                }
            } catch (final Exception e) {
                logDebug(logErrors,
                        "   ❌ Strategy 0 failed: Neodymium Automation ID [[data-neo-ref='{}']] with error: {}", target,
                        e.getMessage());
            }
        }
        ...
        if (target.contains("parentText=")) {
            ...
            if (m.find()) {
                ...
                try {
                    final String parentTextFinderJs = "var expectedParentText = arguments[0];\n" +
                    ...
                    final List<WebElement> matchingEls = Selenide.executeJavaScript(parentTextFinderJs,
                            expectedParentText);
                    if (matchingEls != null && !matchingEls.isEmpty()) {
                        logDebug(logErrors, "   🔍 Resolved using Strategy 0.2: Computed parentText [{}]",
                                expectedParentText);
                        return Selenide.$$(matchingEls);
                    }
                    logDebug(logErrors, "   ❌ Strategy 0.2 failed: Computed parentText [{}]", expectedParentText);
                } catch (final Exception e) {
                    logDebug(logErrors, "   ❌ Strategy 0.2 failed: Computed parentText [{}] with error: {}",
                            expectedParentText, e.getMessage());
                }
            }
        }
        ...
        if (target.contains("::shadow")) {
            try {
                ...
                logDebug(logErrors, "   ❌ Strategy 0.5 failed: Shadow DOM selector [{}] with error: {}", target,
                        e.getMessage());
            }
        }
        ...
        if (!target.startsWith("/") && !target.startsWith("(")) {
            try {
                ...
                logDebug(logErrors, "   ❌ Strategy 1.5 failed: Deep Shadow DOM selector [{}] with error: {}", target,
                        e.getMessage());
            }
        }
        ...
        try {
            ...
            if (extractedTargetName != null && !extractedTargetName.equals(target)) {
                ...
                logDebug(logErrors, "   ❌ Strategy 3 failed (extracted target name): Link text [{}]",
                        extractedTargetName);
            }
        ...
        try {
            ...
            if (extractedTargetName != null && !extractedTargetName.equals(target)) {
                ...
                if (!elements.isEmpty()) {
                    logDebug(logErrors,
                            "   🔍 Resolved using Strategy 4 (extracted target name): Text content XPath [{}]", xpath);
                    return elements;
                }
        ...
        // Strategy 5: Try deep Shadow DOM text search via Javascript (covers what XPath
        // can't)
        try {
            ...
            if (extractedTargetName != null && !extractedTargetName.equals(target)) {
                ...
                if (shadowWebEls != null && !shadowWebEls.isEmpty()) {
                    logDebug(logErrors,
                            "   🔍 Resolved using Strategy 5 (extracted target name): Deep Shadow DOM text search [{}]",
                            extractedTargetName);
                    return Selenide.$$(shadowWebEls);
                }
                logDebug(logErrors, "   ❌ Strategy 5 failed (extracted target name): Deep Shadow DOM text search [{}]",
                        extractedTargetName);
            }
            ...
            if (elementText != null && !elementText.isBlank() && !elementText.equals(target)) {
                ...
                if (shadowWebEls != null && !shadowWebEls.isEmpty()) {
                    logDebug(logErrors, "   🔍 Resolved using Strategy 5: Deep Shadow DOM text search [{}]",
                            elementText);
                    return Selenide.$$(shadowWebEls);
                }
