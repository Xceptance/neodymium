...
/**
 * Captures page context (screenshot + simplified DOM) for the LLM.
 * The DOM is simplified to only include interactive and visible elements
 * to keep token usage low while giving the LLM enough context.
 *
 * All DOM queries are batched into a single JavaScript execution to
 * minimize WebDriver round-trips for performance.
 */
public class PageAnalyzer {
    private static final Logger LOG = LoggerFactory.getLogger(PageAnalyzer.class);

    /**
     * JavaScript that runs in the browser and extracts all needed DOM data
     * in a single execution. Returns a Map with "elements" (list of sections)
     * and "forms" (list of form descriptors).
     *
     * Visibility is checked via offsetParent (null means hidden for
     * non-fixed/non-body elements) and getComputedStyle as fallback.
     */
    ...
    public String captureScreenshot(String title) {
        LOG.debug("Capturing screenshot for: {}", title);
        try {
            String base64 = com.xceptance.neodymium.common.ScreenshotWriter.doScreenshot(title.replaceAll("[^a-zA-Z0-9-]", "_").substring(0, Math.min(title.length(), 12)));
            if (base64 != null) {
                return base64;
            }
        } catch (Exception e) {
            LOG.warn("Could not save screenshot via ScreenshotWriter, falling back to native capture", e);
        }
        return ((org.openqa.selenium.TakesScreenshot) com.xceptance.neodymium.util.Neodymium.getDriver())
                .getScreenshotAs(org.openqa.selenium.OutputType.BASE64);
    }

    /**
     * Captures a simplified representation of the current page's DOM,
     * focusing on interactive and meaningful elements.
     ...
     * Uses a single JavaScript execution for performance.
     *
     * @param forValidation whether to include more text content for validation
     * @return simplified DOM as a structured text
     */
    ...
    /**
     * Formats a single element map into the output string builder.
     * Produces the same text format as the original per-element approach.
     */
