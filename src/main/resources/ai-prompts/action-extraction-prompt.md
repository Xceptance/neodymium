...
## Action Rules
- **Valid Actions**: `BRANCH`, `CLICK`, `TYPE`, `NAVIGATE`, `CLEAR`, `HOVER`, `SCROLL`, `WAIT`, `SELECT`, `KEY_PRESS`, `BACK`, `FORWARD`, `REFRESH`, `ASSERT_EXISTS`, `ASSERT_ABSENT`, `ASSERT_VISIBLE`, `ASSERT_HIDDEN`, `ASSERT_TEXT`, `ASSERT_VALUE`, `ASSERT_ATTRIBUTE`, `ASSERT_COUNT`, `ASSERT_CHECKED`, `ASSERT_UNCHECKED`, `ASSERT_DISABLED`, `ASSERT_ENABLED`, `ASSERT_FOCUSED`, `ASSERT_SELECTED`, `ASSERT_READONLY`, `ASSERT_EDITABLE`, `ASSERT_URL`, `ASSERT_TITLE`.
...
- **ASSERT_EXISTS / ASSERT_ABSENT / ASSERT_VISIBLE / ASSERT_HIDDEN**: Target specific element to verify presence/visibility or absence/dismissal (no 'value').
- **ASSERT_TEXT**: Target element or nearest scoped container (`#checkout-form-container`, `.order-summary`). Set 'value' to expected literal text, number, symbol ('$', '€'), or regex pattern. By default `isRegex: false` (partial match is automatic). Set `isRegex: true` only for dynamic regex patterns (e.g. `INV-[0-9]+`). NEVER target `body`, `html`, or unscoped bare tags (`div`, `span`, `p`, `li`).
...
- **ASSERT_VALUE**: Target input element to verify its entered value attribute. Set 'value' to expected string.
- **ASSERT_ATTRIBUTE**: Target element to verify attribute value (e.g. `value: "placeholder=Search"`) or existence (e.g. `value: "disabled"`).
- **ASSERT_COUNT**: Target element collection selector (e.g. `.cart-item`). Set 'value' to expected count (e.g. `"3"`, `">0"`, `">=1"`).
- **ASSERT_CHECKED / ASSERT_UNCHECKED / ASSERT_DISABLED / ASSERT_ENABLED / ASSERT_FOCUSED / ASSERT_SELECTED / ASSERT_READONLY / ASSERT_EDITABLE**: Target specific element to verify DOM state (no 'value').
- **ASSERT_URL / ASSERT_TITLE**: Target 'url' or 'title'. Set 'value' to expected URL/title or regex pattern.
- **BRANCH**: For conditional instructions ('If X then Y [else Z]'), emit a `BRANCH` action containing: `condition` (array of assertion actions such as `ASSERT_EXISTS`, `ASSERT_ABSENT`, or `ASSERT_TEXT`), `then` (actions array), and optional `else` (actions array).
  * *Faithful Condition*: The condition must reflect the presence/state tested by the instruction (e.g. for "If #field is present / exists / visible", emit `ASSERT_EXISTS` with locator). FORBIDDEN: NEVER invert conditions (e.g. changing to `ASSERT_ABSENT`) based on current page state.
