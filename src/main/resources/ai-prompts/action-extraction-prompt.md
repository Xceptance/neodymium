...
2. **Declarative vs. Imperative Instructions**: Statements describing UI presence, appearance, location, layout, or capabilities are declarative assertions to verify (`ASSERT` or visual check), NOT an imperative command to execute. NEVER emit `CLICK`, `TYPE`, `NAVIGATE`, `CLEAR`, or `SELECT` for declarative statements. If a described element is missing, set 'status' to 'ESCALATE' (or 'FAILED' if at highest context).
...
4. **Visual Checks**: For layout, appearance, colors, or instructions with `(visual)`, inspect visual state and return empty 'actions' with `status: "SUCCESS"` or `"FAILED"`. If visual context is needed or text is fragmented across elements, set 'status': 'ESCALATE' with 'targetContextLevel': '[NEXT_ESCALATION]'.
