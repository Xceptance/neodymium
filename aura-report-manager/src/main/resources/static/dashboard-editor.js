...
function runReviewSteps() {
    ...
    .then(data => {
        ...
        if (data.suggestions && data.suggestions.length > 0) {
                ...
                if (targetRow) {
                    ...
                    card.innerHTML = `
                        <div class="suggestion-left">
                            ...
                            <span><b>Aura Step Review:</b> ${sugg.message}</span>
                        </div>
            ...
            showToast(`✨ Step Review complete: ${data.suggestions.length} suggestion(s) found`, "info");
        } else {
            showToast("✅ Step Review complete: All steps look great! No suggestions needed.", "success");
        }
    })
    .catch(err => {
        console.error('Failed to run step review:', err);
        if (reviewBtn) {
            reviewBtn.innerHTML = '<span class="material-symbols-outlined">auto_awesome</span> Review Steps';
            reviewBtn.disabled = false;
        }
